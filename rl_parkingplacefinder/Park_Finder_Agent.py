...
class Park_Finder_Agent():
    ...
    def get_parking_lot_width(self):
        ...
        single_connection_list = [item for item, count in collections.Counter(
            connection_list).items() if count == 1]
    ...
    def step(self, action):
        ...
        if reward == 0:
            print("something went wrong with the reward.. {} -> {}, action: {}".format(
                self.agentPosition, resultingState, action))
        else:
